...
const mygetData = async () => {
  ...
  console.log("api/user/list?page=1&token="+user_token,"url");
  // const res = await getData(/*"api/user/detail/4"*/"api/user/list?page=1&token="+user_token);
  // const res = await getData("/api/theme/list/1");
  const res = await getData("api/user/list/?page=1&token="+user_token);
  // "api/user/list?page=1&token="+user_token
  ...
  filterData.value = filterData.value.map(item => ({
    ...
    username:formatName(item.username)
...
function formatName(username){
  username=JSON.parse(localStorage.getItem('login-param')).username
}
